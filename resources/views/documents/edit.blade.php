<x-app-layout>
    <x-slot name="header">
        <h2 class="font-serif" style="font-size: 1.875rem; font-style: italic;">{{ __('Edit Document') }}</h2>
    </x-slot>

    <div class="editorial-section">
        <div class="container-xl px-4 px-md-5">
            <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data" style="max-width: 640px;">
                @csrf
                @method('PUT')
                ...
                <div class="editorial-card">
                    ...
                    <div class="mb-3">
                        ...
                        <x-text-input id="title" name="title" type="text" class="d-block w-100" :value="old('title', $document->title)" required autofocus />
                    ...
                    <div class="mb-3">
                        ...
                        <x-text-input id="document_number" name="document_number" type="text" class="d-block w-100" :value="old('document_number', $document->document_number)" />
                        <x-input-error :messages="$errors->get('document_number')" />
                    </div>

                    <div class="mb-3">
                        <x-input-label for="description" :value="__('Description')" />
                        <textarea id="description" name="description" rows="3" class="editorial-input">{{ old('description', $document->description) }}</textarea>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            ...
                            <x-text-input id="issue_date" name="issue_date" type="date" class="d-block w-100" :value="old('issue_date', $document->issue_date?->format('Y-m-d'))" />
                            <x-input-error :messages="$errors->get('issue_date')" />
                        </div>
                        <div class="col-6">
                            <x-input-label for="expiration_date" :value="__('Expiration Date')" />
                            <x-text-input id="expiration_date" name="expiration_date" type="date" class="d-block w-100" :value="old('expiration_date', $document->expiration_date?->format('Y-m-d'))" />
                            <x-input-error :messages="$errors->get('expiration_date')" />
                        </div>
                    </div>

                    <div class="form-check mb-3">
                        <input type="checkbox" name="is_favorite" value="1" {{ old('is_favorite', $document->is_favorite) ? 'checked' : '' }} class="form-check-input editorial-checkbox" id="is_favorite">
                ...
                <div class="editorial-card mt-3">
                    ...
                    @if($document->getMedia('document-images')->count() > 0)
                        <div class="mb-3">
                            <span class="editorial-label">Current Images</span>
                            <div class="d-flex gap-3 mt-2">
                                @foreach($document->getMedia('document-images') as $media)
                                    <div style="width: 96px; height: 96px; border: 1px solid var(--border); overflow: hidden;">
                                        <img src="{{ $media->getUrl() }}" alt="{{ $media->name }}" class="w-100 h-100 image-editorial" style="object-fit: cover;">
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="d-flex justify-content-between align-items-center mt-4">
                    <a href="{{ route('documents.show', $document) }}" class="editorial-link text-decoration-underline" style="letter-spacing: normal;">
                        {{ __('Cancel') }}
                    </a>
